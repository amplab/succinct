...
public class DeltaEncodedIntVectorOps {
   ...
   * @param flag Whether to get lower bound or upper bound; true - lower bound; false - upper
   * bound.
  ...
  public static int binarySearch(ByteBuffer vector, int val, int startIdx, int endIdx,
    ...
    int sampleOffset =
        binarySearchSamples(samples, sampleBits, val, startIdx / samplingRate,
            endIdx / samplingRate);
    ...
    while (deltaSum < val && deltaIdx < deltaLimit) {
      int block = (int) BitVectorOps.getValue(deltas, currentDeltaOffset, 16);
      int cnt = EliasGamma.PrefixSum.count(block);
      int blockSum = EliasGamma.PrefixSum.sum(block);
      ...
      if (cnt == 0) {
        ...
        while (BitVectorOps.getBit(deltas, currentDeltaOffset) != 1) {
          deltaWidth++;
          currentDeltaOffset++;
          if (!BitVectorOps.checkIndex(deltas, currentDeltaOffset)) {
            breakOuterLoop = true;
            break;
          }
        }
      ...
      } else if (deltaSum + blockSum < val && deltaIdx + cnt < deltaLimit) {
        // If sum can be computed from the prefixsum table
        deltaSum += blockSum;
        currentDeltaOffset += EliasGamma.PrefixSum.offset(block);
        deltaIdx += cnt;
