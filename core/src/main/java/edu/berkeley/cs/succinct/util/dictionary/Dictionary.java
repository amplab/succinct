...
public class Dictionary {
  ...
  public Dictionary(BitMap bitMap) {
    ...
    for (i = 0; i < bitMap.size; i++) {
      ...
      if (i % 64 == 0) {
        p = (short) CommonUtils.popCount((bitMap.data[i / 64] >> 48) & 65535);
        p = (short) (p % 16);
        compSize += Tables.offsetBits[p];
        dict.add(p);
        dict.add(Tables.encodeTable.get(p).get((int) ((bitMap.data[i / 64] >> 48) & 65535)));

        p = (short) CommonUtils.popCount((bitMap.data[i / 64] >> 32) & 65535);
        p = (short) (p % 16);
        compSize += Tables.offsetBits[p];
        dict.add(p);
        dict.add(Tables.encodeTable.get(p).get((int) ((bitMap.data[i / 64] >> 32) & 65535)));

        p = (short) CommonUtils.popCount((bitMap.data[i / 64] >> 16) & 65535);
        p = (short) (p % 16);
        compSize += Tables.offsetBits[p];
        dict.add(p);
        dict.add(Tables.encodeTable.get(p).get((int) ((bitMap.data[i / 64] >> 16) & 65535)));

        p = (short) CommonUtils.popCount(bitMap.data[i / 64] & 65535);
        p = (short) (p % 16);
        compSize += Tables.offsetBits[p];
        dict.add(p);
        dict.add(Tables.encodeTable.get(p).get((int) (bitMap.data[i / 64] & 65535)));
    ...
    for (i = 0; i < dict.size(); i++) {
      if (i % 2 == 0) {
        ...
        this.bitMap.setValPos((int) numBits, dict.get(i), Tables.offsetBits[dict.get(i - 1)]);
  ...
  public long getRank1(int i) {
    ...
    while (l1Idx >= 16) {
      ...
      pos += 4;
      blockOffset = (int) ((blockClass == 0) ? bitMap.getBit((int) pos) * 16 : 0);
    ...
    blockClass = (int) bitMap.getValPos((int) pos, 4);
    pos += 4;
    blockOffset = (int) bitMap.getValPos((int) pos, Tables.offsetBits[blockClass]);
  ...
  public ByteBuffer getByteBuffer() {
    int dSize =
      8 * (1 + rankL3.length + rankL12.length + posL3.length + posL12.length + bitMap.data.length
        + 1);
