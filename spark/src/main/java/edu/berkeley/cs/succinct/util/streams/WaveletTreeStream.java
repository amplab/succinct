...
import static edu.berkeley.cs.succinct.util.CommonUtils.DictionaryUtils.GETPOSL1;
import static edu.berkeley.cs.succinct.util.CommonUtils.DictionaryUtils.GETRANKL1;
...
public class WaveletTreeStream {
    ...
    private long waveletTreeLookup(int contextPos, int cellPos, int startIdx, int endIdx) throws IOException {
    ...
    private long select0(long dictPos, int i) throws IOException {
        ...
        while (true) {
            ...
            pos += 4;
            blockOffset = (int) ((blockClass == 0) ? SerializedOperations.BitMapOps.getBit(dictBuf, pos) * 16
                    : 0);
            pos += offsetSize;
        ...
        blockClass = (int) SerializedOperations.BitMapOps.getValPos(dictBuf, pos, 4);
        pos += 4;
        blockOffset = (int) SerializedOperations.BitMapOps.getValPos(dictBuf, pos,
