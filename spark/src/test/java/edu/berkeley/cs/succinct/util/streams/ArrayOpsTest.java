...
public class ArrayOpsTest extends TestCase {
  ...
  public void testGetRank1() throws Exception {
    ...
    assertEquals(SerializedOperations.ArrayOps.getRank1(ls, 0, data.length, 4L), 2L);
    assertEquals(SerializedOperations.ArrayOps.getRank1(ls, 0, data.length, 6L), 3L);
    assertEquals(SerializedOperations.ArrayOps.getRank1(ls, 0, data.length, 22L), 8L);
    assertEquals(SerializedOperations.ArrayOps.getRank1(ls, 0, data.length, 29L), 10L);
    assertEquals(SerializedOperations.ArrayOps.getRank1(ls, 0, data.length, 33L), 10L);
    is.close();
  }
}
